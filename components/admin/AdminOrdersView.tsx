...
export function AdminOrdersView({ orders }: { orders: AdminOrderRow[] }) {
  ...
  const renderRows = (rows: AdminOrderRow[], emptyLabel: string) => {
    if (rows.length === 0) {
      return (
        <tr>
          <td colSpan={7} className="px-6 py-6 text-center text-sm text-muted">{emptyLabel}</td>
        </tr>
      );
    }
    return rows.map((o) => {
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[46rem] table-fixed text-sm">
            ...
            <tbody>
              <SectionRow icon={Clock} tone="text-brand-sky" label="Active orders" count={active.length} />
              {renderRows(active, "No active orders.")}
              <SectionRow icon={CheckCircle2} tone="text-emerald-600" label="Completed orders" count={completed.length} />
              {renderRows(completed, "No completed orders yet.")}
            </tbody>
          </table>
        </div>
      </div>
...
function SectionRow({
  ...
  return (
    <tr className="border-b border-brand-blue/[0.08] bg-brand-blue/[0.04]">
      <td colSpan={7} className="px-6 py-3">
        <span className="inline-flex items-center gap-2 font-sans text-sm font-bold text-ink">
          <Icon className={`h-4 w-4 ${tone}`} />
          {label}
          <span className="rounded-full bg-white px-2 py-0.5 text-xs font-medium text-muted">{count}</span>
        </span>
      </td>
    </tr>
  );
