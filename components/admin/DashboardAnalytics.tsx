...
import {
  DollarSign, CalendarRange, ShoppingBag, TrendingUp, Timer, Clock, CheckCircle2, Users, Repeat, Crown,
  type LucideIcon,
} from "lucide-react";
...
const OPTIONS: { key: Key; label: string; cur: string; prev: string }[] = [
  { key: "week", label: "This week", cur: "This week", prev: "Last week" },
  { key: "month", label: "This month", cur: "This month", prev: "Last month" },
  { key: "year", label: "This year", cur: "This year", prev: "Last year" },
];
...
/** Smooth spline with control points clamped to the plot area so the curve
...
export function DashboardAnalytics({ metrics, periods }: { metrics: DashboardMetrics; periods: Periods }) {
  ...
  const areaFrom = (path: string) => `${path} L ${x(n - 1)},${CHART_BOTTOM} L ${x(0)},${CHART_BOTTOM} Z`;
  ...
  const kpis: { icon: LucideIcon; label: string; value: string; tone: string }[] = [
    { icon: DollarSign, label: "Daily revenue", value: metrics.dailyRevenue, tone: "red" },
    { icon: CalendarRange, label: "Monthly revenue", value: metrics.monthlyRevenue, tone: "red" },
    { icon: ShoppingBag, label: "Today's orders", value: String(metrics.todayOrders), tone: "blue" },
    { icon: TrendingUp, label: "Avg order value", value: metrics.avgOrderValue, tone: "blue" },
    { icon: Timer, label: "Processing time", value: metrics.processingDays, tone: "blue" },
    { icon: Clock, label: "Active orders", value: String(metrics.activeOrders), tone: "blue" },
    { icon: CheckCircle2, label: "Completed orders", value: String(metrics.completedOrders), tone: "green" },
    { icon: Users, label: "Total users", value: String(metrics.totalUsers), tone: "blue" },
    { icon: Repeat, label: "Returning rate", value: `${metrics.returningRate}%`, tone: "green" },
    { icon: Crown, label: "Best-selling product", value: metrics.bestSeller, tone: "red" },
  ];

  return (
    <div className="space-y-6">
      {/* Header with the period selector in the top-right */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="font-sans text-2xl font-bold text-ink">Dashboard</h1>
          <p className="text-sm text-muted">Store analytics for the selected timeframe.</p>
        </div>
        <select
          value={key}
          onChange={(e) => {
        ...
        {kpis.map((k) => {
          ...
          return (
            <div key={k.label} className="card p-5">
              ...
              <p className="mt-3 text-sm text-muted">{k.label}</p>
              <p className="font-sans text-xl font-bold text-ink">{k.value}</p>
            </div>
          );
        })}
      </div>
          ...
          <div className="relative mt-4">
            ...
            >
              <defs>
                ...
                <linearGradient id="prev-fill" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#6E9CC4" stopOpacity="0.16" />
              ...
              {d.labels.map((_, i) => (
                <line key={`v-${i}`} x1={x(i)} y1={padT} x2={x(i)} y2={CHART_BOTTOM} stroke="#1B3454" strokeOpacity="0.07" />
              ))}

              <path d={areaFrom(prevPath)} fill="url(#prev-fill)" />
              <path d={prevPath} fill="none" stroke="#6E9CC4" strokeWidth="1.75" strokeLinecap="round" />
              <path d={areaFrom(curPath)} fill="url(#cur-fill)" />
              <path d={curPath} fill="none" stroke="#E85A4F" strokeWidth="1.75" strokeLinecap="round" />

              {hover !== null && (
                <g>
                  <line x1={x(hover)} y1={padT} x2={x(hover)} y2={CHART_BOTTOM} stroke="#1B3454" strokeOpacity="0.25" />
                  <circle cx={x(hover)} cy={y(d.previous[hover])} r="3.5" fill="#6E9CC4" stroke="#fff" strokeWidth="1.5" />
                  <circle cx={x(hover)} cy={y(d.current[hover])} r="3.5" fill="#E85A4F" stroke="#fff" strokeWidth="1.5" />
                </g>
              )}
            ...
            {hover !== null && (
              <div
                ...
                style={{ left: `${(x(hover) / W) * 100}%`, top: `${(Math.min(y(d.current[hover]), y(d.previous[hover])) / H) * 100}%` }}
              >
